...
type CmdbHost struct {
	...
	HostName    string     `gorm:"column:host_name;type:varchar(64);comment:'名称';NOT NULL" json:"hostName"`
	GroupID     uint       `gorm:"column:group_id;comment:'分组ID';NOT NULL" json:"groupId"`
	Group       CmdbGroup  `gorm:"foreignKey:GroupID" json:"group"`
	PrivateIP   string     `gorm:"column:private_ip;type:varchar(64);comment:'私网IP'" json:"privateIp"`
	PublicIP    string     `gorm:"column:public_ip;type:varchar(64);comment:'公网IP'" json:"publicIp"`
	SSHIP       string     `gorm:"column:ssh_ip;type:varchar(64);comment:'SSH连接IP';NOT NULL" json:"sshIp"`
	SSHName     string     `gorm:"column:ssh_name;type:varchar(64);comment:'SSH用户名'" json:"sshName"`
	...
	Vendor      int        `gorm:"column:vendor;type:varchar(32);comment:'1->自建,2->阿里云,3->腾讯云'" json:"vendor"`
	Region      string     `gorm:"column:region;type:varchar(64);comment:'区域'" json:"region"`
	InstanceID  string     `gorm:"column:instance_id;type:varchar(128);comment:'实例ID'" json:"instanceId"`
	Name        string     `gorm:"column:name;type:varchar(64);comment:'ecs主机名称';NOT NULL" json:"name"`
	OS          string     `gorm:"column:os;type:varchar(128);comment:'操作系统'" json:"os"`
	Status      int        `gorm:"column:status;comment:'状态:1->认证成功,2->未认证,3->认证失败'" json:"status"`
	CPU         string     `gorm:"column:cpu;type:varchar(32);comment:'CPU信息'" json:"cpu"`
	Memory      string     `gorm:"column:memory;type:varchar(32);comment:'内存信息'" json:"memory"`
	Disk        string     `gorm:"column:disk;type:varchar(128);comment:'磁盘信息'" json:"disk"`
	BillingType string     `gorm:"column:billing_type;type:varchar(32);comment:'计费方式'" json:"billingType"`
